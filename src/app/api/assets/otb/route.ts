...
export async function POST(req: Request) {
    ...
    try {
        ...
        const newOtb = await prisma.otb.create({
            data: {
                name,
        ...
        return NextResponse.json(newOtb, { status: 201 });
    } catch (error: any) {
        if (error.code === 'P2002') {
            return NextResponse.json({ error: 'An OTB with this name already exists' }, { status: 409 });
        }
        return NextResponse.json({ error: error.message }, { status: 500 });
    }
}
